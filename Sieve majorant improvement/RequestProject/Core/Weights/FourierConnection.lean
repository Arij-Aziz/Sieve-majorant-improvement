...
lemma sieveMajorant_l2NormSq (d m : ℕ) (hd : 2 ≤ d) (hm : 1 ≤ m)
    ...
    (sieveMajorant d m hN).l2NormSq = (d * m : ℝ) - 3 * (m : ℝ) / 4 := by
      ...
      unfold sieveMajorantFun sieveTargetFun Majorant.l2NormSq; norm_num [ Finset.sum_ite ] ; ring;
      rw [ Finset.filter_not, Finset.card_sdiff ] ; norm_num [ this ] ; ring;
...
theorem sieveMajorant_l2NormSq_eq_selbergForm
    ...
    (sieveMajorant d m hN).l2NormSq / (d * m) =
      1 - 3 / (4 * (d : ℝ)) := by
  convert congr_arg ( fun x : ℝ => x / ( d * m ) ) ( sieveMajorant_l2NormSq d m hd hm ( by positivity ) ) using 1 ; ring;
